...
\begin{prp}
Suppose Assumption \ref{Assump1} holds. Aggregate welfare is weakly decreasing in $(1-\eta)\tau_y$ and strictly decreasing over any range of policies whose implied thresholds lie in the support of $G$.
\end{prp}
\begin{proof}
From (\ref{eq:nthreshold}) and Assumption \ref{Assump1}, $\underline w_y$ increases with $(1-\eta)\tau_y$.
Aggregate welfare is given by Equation (\ref{eq:aggW}). Since $G$ is weakly increasing and $\mathcal{N}>0$, an increase in $(1-\eta)\tau_y$ weakly decreases $W$ by increasing the measure of young individuals who cannot finance the alternative investment. Since $G$ is strictly increasing on its support, the decrease is strict whenever the implied thresholds lie in that support.
\end{proof}
