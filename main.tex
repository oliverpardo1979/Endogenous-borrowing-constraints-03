...
\end{prp}
\begin{proof}
If $F>\mathcal{N}$, then $\underline w_y$ is increasing in $(1 - \eta)\tau_y$. Aggregate welfare is given by Eqaution (\ref{eq:aggW}).
Since $G$ is weakly increasing and $\mathcal{N}>0$, an increase in $(1-\eta)\tau_y$ weakly decreases $W$ by increasing the measure of young individuals who cannot finance the alternative investment. The decrease is strict whenever $G$ has positive mass around the threshold.
\end{proof}
