...
\begin{prp}
Suppose Assumption \ref{Assump1} holds and let $\omega=(1-\eta)\tau_y$. Aggregate welfare is weakly decreasing in $\omega$. If $G$ is differentiable with strictly positive density on the relevant range of $\underline w_y(\omega)$, then aggregate welfare is strictly decreasing in $\omega$ on that range.
\end{prp}
\begin{proof}
From (\ref{eq:nthreshold}),
\[
\underline w_y(\omega)=\frac{F-\mathcal{N}}{1-\omega}.
\]
Assumption \ref{Assump1} implies $F>\mathcal{N}$, so
\[
\frac{\partial \underline w_y(\omega)}{\partial \omega}
=\frac{F-\mathcal{N}}{(1-\omega)^2}>0.
\]
Aggregate welfare is given by Equation (\ref{eq:aggW}). Since $G$ is weakly increasing and $\mathcal{N}>0$, an increase in $\omega$ weakly decreases $W$ by increasing the measure of young individuals who cannot finance the alternative investment. If $G$ is differentiable with density $g(\underline w_y(\omega))>0$, then
\[
\frac{\partial W}{\partial \omega}
=-\mathcal{N}g(\underline w_y(\omega))\frac{F-\mathcal{N}}{(1-\omega)^2}<0,
\]
which gives strict monotonicity on that range.
\end{proof}
