...
\begin{dfn}
Given policy parameters $(\tau_y,\tau_m,\eta)$, an equilibrium consists of borrowing, investment, consumption, and saving choices for individuals and a lending rate schedule $R(v)$ such that: individuals maximize expected lifetime payoff subject to the budget constraints (\ref{eq:nbca})--(\ref{eq:nbco}) and limited liability; borrowers repay when $z=\pi$ and default when $z=0$; lenders break even against the safe asset for every collateral coverage ratio $v$ at which they lend, so that (\ref{eq:narby}) holds; and young individuals invest in the alternative asset whenever they can finance $F$ and satisfy the incentive-compatibility constraint (\ref{eq:nicc}), equivalently $b(w_y)\leq \mathcal{N}+\eta B(w_y)$.
\end{dfn}
