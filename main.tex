...
\begin{prp}
Suppose Assumption \ref{Assump1} holds. Aggregate welfare is weakly decreasing in $(1-\eta)\tau_y$. If $G$ is differentiable with strictly positive density on the relevant range of $\underline w_y$, then aggregate welfare is strictly decreasing in $(1-\eta)\tau_y$ on that range.
\end{prp}
\begin{proof}
...
\[
\frac{\partial \underline w_y}{\partial ((1-\eta)\tau_y)}
=\frac{F-\mathcal{N}}{[1-(1-\eta)\tau_y]^2}>0.
\]
Aggregate welfare is given by Equation (\ref{eq:aggW}). Since $G$ is weakly increasing and $\mathcal{N}>0$, an increase in $(1-\eta)\tau_y$ weakly decreases $W$ by increasing the measure of young individuals who cannot finance the alternative investment. If $G$ is differentiable with density $g(\underline w_y)>0$, then
\[
\frac{\partial W}{\partial ((1-\eta)\tau_y)}
=-\mathcal{N}g(\underline w_y)\frac{F-\mathcal{N}}{[1-(1-\eta)\tau_y]^2}<0,
\]
