...
\begin{prp}
Suppose Assumption \ref{Assump1} holds. Aggregate welfare is weakly decreasing in $(1-\eta)\tau_y$. The decrease is strict between any two values of $(1-\eta)\tau_y$ whenever $G$ assigns positive mass to the interval between the two implied thresholds.
\end{prp}
\begin{proof}
...
\]
Aggregate welfare is given by Equation (\ref{eq:aggW}). Since $G$ is weakly increasing and $\mathcal{N}>0$, an increase in $(1-\eta)\tau_y$ weakly decreases $W$ by increasing the measure of young individuals who cannot finance the alternative investment. For two policy values that imply thresholds $\underline w_y'$ and $\underline w_y''>\underline w_y'$, the welfare change is $-\mathcal{N}\left[G(\underline w_y'')-G(\underline w_y')\right]$. This change is strictly negative whenever $G(\underline w_y'')>G(\underline w_y')$.
\end{proof}
